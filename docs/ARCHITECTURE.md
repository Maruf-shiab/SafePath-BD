...
## Current First-Milestone Implementation Note (Chunk 5)

The current application now implements the initial in-app notification architecture without changing the database schema.

- `INotificationService` / `NotificationService` owns user-scoped notification reads, unread counts, pagination and read-state changes.
- Moderation remains the authoritative owner of report status transitions. Successful moderation decisions queue the reporter notification inside the same EF Core unit of work / transaction.
- Notification lookup IDs are never hard-coded. The service resolves the existing `notification_types.type_code` values.
- Existing lookup codes `REPORT_VERIFIED`, `REPORT_REJECTED` and `REPORT_RESOLVED` are used directly. Because the existing lookup table has no dedicated `NEEDS_INFO` or `DUPLICATE` code, those two outcomes use the seeded `SYSTEM` type. No lookup data or schema is created by application code.
- `NEEDS_INFO` may surface the review note as the reporter-facing information request. Other moderation notes remain staff audit history and are not copied into reporter notifications.
- `NotificationsController` derives the user from `ClaimsPrincipal`; notification ownership is part of every read/update predicate.
- Read mutations are POST + antiforgery protected. Action links are generated by the server and target local report-detail routes.
- The navbar bell is server-rendered with an initial unread count and refreshes through lightweight HTTP endpoints. Automatic refresh uses controlled polling, not push/SignalR and is not described as real-time.

Dashboard projections are owned by `IDashboardService` / `DashboardService`:

- User dashboard: current-user report status aggregates, recent reports, `NEEDS_INFO` action-required items, unread/recent notifications and an activity feed composed from existing records.
- Moderator dashboard: operational status totals, open queue preview and real verification history.
- Admin dashboard: real user totals, active accounts, report-type totals, report-status totals, open queue preview and recent `admin_actions`.
- Read-heavy dashboard queries use `AsNoTracking`, database aggregation/projection and bounded recent-item limits.

The safety-score engine, routing provider, route-to-segment matching, saved routes/places and advanced analytics remain second-half features and are intentionally absent from this milestone.
